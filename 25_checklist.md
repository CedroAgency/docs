...
Вёрстка должна максимально соответствовать макету. Должны совпадать размеры элементов, отступы между блоками, цвета, шрифты и т.п.

Допустимы отличия, связанные с:
...
Для проверки можно использовать расширение [PerfectPixel](https://chrome.google.com/webstore/detail/perfectpixel-by-welldonec/dkaagdgjmgdmbnecmcefdhjekcoceebi?hl=ru)

## Проверка кода валидаторами
[Проверяем CSS код](https://jigsaw.w3.org/css-validator/)

[Проверяем HTML код](https://validator.w3.org/unicorn/)

[Проверяем БЭМ структуру](https://yoksel.github.io/html-tree/)
...
* Windows
    * Chrome
    * Firefox
    * Yandex (опционально, русскоязычный сегмент)
* macOS
    * Safari
    * Chrome (опционально)
* Android
...
* Android
    * Любое мобильное устройство и планшет с актуальной версией системы
        * Chrome
* iOS
...
На сайте должны быть указаны метатеги (`title`, `description`, `og`).
...
Выполняем рекомендации [Google Lighthouse](https://chromewebstore.google.com/detail/lighthouse/blipmdconlkpinefehnmjammfjpmpbjk)

Сайт должен находится в "зелёной зоне".
